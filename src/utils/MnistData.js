...
export default class MnistData {
  ...
  async load() {
    ...
    this.trainIndices = this.tf.util.createShuffledIndices(NUM_TRAIN_ELEMENTS);
    this.testIndices = this.tf.util.createShuffledIndices(NUM_TEST_ELEMENTS);
  ...
  nextBatch(batchSize, data, index) {
    ...
    const xs = this.tf.tensor2d(batchImagesArray, [batchSize, IMAGE_SIZE]);
    const labels = this.tf.tensor2d(batchLabelsArray, [batchSize, NUM_CLASSES]);
